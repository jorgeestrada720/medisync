...
# 1 "./servo.h" 1
# 23 "./servo.h"
const uint16_t pulseMin[6] = {500 ,500 ,500 ,500 ,500 ,500 };
const uint16_t pulseMax[6] = {2500,2500,2500,2500,2500,2500};

uint16_t pulse[6]={50,50,50,50,50,50};
uint16_t TMR0_ini;
uint8_t servo_active=1;
...
uint16_t ProbarServo(uint8_t pinDecrementar, uint8_t pinIncrementar);
...
void __attribute__((picinterrupt(("")))) Timer1_ISR()
{
    if (TMR1IF == 1) {
        ...
        switch (servo_active){
          case 1: LATCbits.LATC1=!LATCbits.LATC1;signalServo(LATCbits.LATC1,pulse[0]);break;
          case 2: LATBbits.LATB1=!LATBbits.LATB1;signalServo(LATBbits.LATB1,pulse[1]);break;
          case 3: LATBbits.LATB2=!LATBbits.LATB2;signalServo(LATBbits.LATB2,pulse[2]);break;
          case 4: LATBbits.LATB3=!LATBbits.LATB3;signalServo(LATBbits.LATB3,pulse[3]);break;
          case 5: LATBbits.LATB4=!LATBbits.LATB4;signalServo(LATBbits.LATB4,pulse[4]);break;
          case 6: LATBbits.LATB5=!LATBbits.LATB5;signalServo(LATBbits.LATB5,pulse[5]);break;
         }
...
void turn_valve(int direction){
    ...
    if(direction == forward){
        ...
        servo1_write(140); _delay((unsigned long)((2000)*(8000000L/4000.0)));

        servo1_write(45);_delay((unsigned long)((2000)*(8000000L/4000.0)));
...
int handle_actuator_push(int act_pos){
    ...
    if(decision == 1){
        ...
        while(act_pos > 20){
            ...
            _delay((unsigned long)((25)*(8000000L/4000.0)));
...
long calculateBitRatePer10Sec(long volume_mL, long totalSeconds) {
    ...
    long length_mm_scaled = (volume_mL * 10000) / 158;


    long length_per_min_mm_scaled = length_mm_scaled / totalSeconds;


    long bit_rate_per_min_scaled = (length_per_min_mm_scaled * 100) / (635/60);
...
void infusion(){
    ...
    while( (mil) < stopTime*1000){
        ...
        controlSignal = (Kp * error + Ki * errorSum + Kd * dError) / 100;
...
void main(void) {
    ...
    while (1) {
        ...
        currentState = GetNextState(currentState, userInput);
    }
}
